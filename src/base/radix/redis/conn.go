...
func (c *conn) init() *Error {
	...
	c.write_buff = bytes.NewBuffer(make([]byte, 0, 4096))
...
func newBroomConn(config *Config) (c *conn, err *Error) {
	c = &conn{
		closed_: 1, // closed by default
		config:  config,
	}
	if err = c.initBroom(); err != nil {
		c.close()
		c = nil
	}
	return
...
func (c *conn) writeRequest(calls ...call) *Error {
	c.setWriteTimeout()
	c.write_buff.Reset()
	err := createRequestWithBuff(c.write_buff, calls...)
	if err != nil {
		return newError("create request error: " + err.Error())
	}
	if _, err := c.conn.Write(c.write_buff.Bytes()); err != nil {
		panic(err.Error())
		/*
...
func (c *conn) DirectCall(msg []byte) *Reply {
